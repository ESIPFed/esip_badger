(function(){
  ...
  function search(){
    ...
    for (var i = 0; i < scripts.length; i++) {
      ...
      if (/\/esip_badger\.js(\?.*)?$/.test(script.src)) {
  ...
  function replace(script){
    ...
    iframe.src = script.src.replace(/\/esip_badger\.js.*/, iframePath);
    iframe.style.borderWidth = 0;
    iframe.className = '__esipbadger';
  ...
  function setup(iframe){
    ...
    iframe.contentWindow.postMessage('esipbadger:' + id, '*');
    window.addEventListener('message', function(e){
      // show dialog upon click
      if ('badger-click:' + id  == e.data) {
        showDialog(iframe);
      }

      // update width
      var wp = 'badger-width:' + id + ':';
  ...
  function showDialog(iframe){
    ...
    function click(ev){
      if ('__esipbadger' != ev.target.className) {
        hide();
      }
    }
